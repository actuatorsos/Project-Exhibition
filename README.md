...
- **vote.html** — the public voting page: voters tap **Pick** on exactly **3** projects, then submit their ballot with the one-time code from their ticket. Live results (bars, medals, podium) update on the page.
- **codes.html** — teacher-only ticket maker: generates unique one-time codes, prints cut-out tickets (each with a QR that opens vote.html with the code pre-filled), and produces the SQL to activate the codes in Supabase.
...
3. They pick 3 different projects and press **Send my 3 votes**.
...
## Security
- One ballot per code, exactly 3 distinct picks, enforced **server-side** in `cast_ballot` (atomic, race-safe).
